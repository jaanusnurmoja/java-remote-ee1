...
import ee.sda.training.config.KafkaProperties;
...
class JavaRemoteEe1KafkaResourceIT {
    ...
    void setup() {
        ...
        JavaRemoteEe1KafkaResource kafkaResource = new JavaRemoteEe1KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/java-remote-ee-1-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/java-remote-ee-1-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
